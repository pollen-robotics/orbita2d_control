...
class KinematicsModel:
    ...
    def forward_position(self, angle_a: float, angle_b: float) -> (float, float):
        ring = _orbita2d.ffi.new("double *")
        center = _orbita2d.ffi.new("double *")
    ...
    def forward_velocity(self, velocity_a: float, velocity_b: float) -> (float, float):
        ring_velocity = _orbita2d.ffi.new("double *")
        center_velocity = _orbita2d.ffi.new("double *")
    ...
    def forward_torque(self, torque_a: float, torque_b: float) -> (float, float):
        ring_torque = _orbita2d.ffi.new("double *")
        center_torque = _orbita2d.ffi.new("double *")
...
class Orbita2dController:
    ...
    def from_config(cls, configfile: str):
        uid = _orbita2d.ffi.new("uint32_t *")
        _orbita2d.lib.orbita2d_controller_from_config(configfile.encode(), uid)
    ...
    def get_current_orientation(self) -> (float, float):
        angle = _orbita2d.ffi.new("double(*)[2]")
        _orbita2d.lib.orbita2d_get_current_orientation(self.uid, angle)
    ...
    def get_target_orientation(self) -> (float, float):
        angle = _orbita2d.ffi.new("double(*)[2]")
        _orbita2d.lib.orbita2d_get_target_orientation(self.uid, angle)
        return angle[0][0], angle[0][1]

    def set_target_orientation(self, target: (float, float)):
        arr = _orbita2d.ffi.new("double(*)[2]", target)
        arr[0][0] = target[0]
        arr[0][1] = target[1]
        _orbita2d.lib.orbita2d_set_target_orientation(self.uid, arr)

    def get_raw_motors_velocity_limit(self) -> (float, float):
        limit = _orbita2d.ffi.new("double(*)[2]")
        _orbita2d.lib.orbita2d_get_raw_motors_velocity_limit(self.uid, limit)
        return limit[0][0], limit[0][1]

    def set_raw_motors_velocity_limit(self, limit: (float, float)):
        arr = _orbita2d.ffi.new("double(*)[2]", limit)
        arr[0][0] = limit[0]
        arr[0][1] = limit[1]
        _orbita2d.lib.orbita2d_set_raw_motors_velocity_limit(self.uid, arr)

    def get_raw_motors_torque_limit(self) -> (float, float):
        limit = _orbita2d.ffi.new("double(*)[2]")
        _orbita2d.lib.orbita2d_get_raw_motors_torque_limit(self.uid, limit)
        return limit[0][0], limit[0][1]
    ...
    def set_raw_motors_torque_limit(self, limit: (float, float)):
        ...
        arr[0][0] = limit[0]
        arr[0][1] = limit[1]
        _orbita2d.lib.orbita2d_set_raw_motors_torque_limit(self.uid, arr)

    def get_raw_motors_pid_gains(
        self,
    ) -> ((float, float, float), (float, float, float)):
        pid = _orbita2d.ffi.new("double(*)[6]")
        _orbita2d.lib.orbita2d_get_raw_motors_pid_gains(self.uid, pid)
    ...
    def set_raw_motors_pid_gains(
        ...
        arr = _orbita2d.ffi.new("double(*)[6]")
        arr[0][0] = pid[0][0]
        arr[0][1] = pid[0][1]
        arr[0][2] = pid[0][2]
        arr[0][3] = pid[1][0]
        arr[0][4] = pid[1][1]
        arr[0][5] = pid[1][2]
        _orbita2d.lib.orbita2d_set_raw_motors_pid_gains(self.uid, arr)
